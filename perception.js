(function () {
  ...
  function renderUserProfileTab() {
    ...
    container.innerHTML = `
      ...
      <article class="fcu-account-card">
        <div>
          ...
          <p class="fcu-account-card-desc">Atualize sua senha de acesso a qualquer momento. A nova senha deve ter no mínimo 8 caracteres.</p>
          <form id="fcu-pwd-form">
            <div class="fcu-acc-field">
              <label for="fcu-new-pwd-input">Nova senha de acesso</label>
              <input id="fcu-new-pwd-input" type="password" name="new_pwd" minlength="8" required placeholder="Preencha sua nova senha">
            </div>
    ...
    $('#fcu-pwd-form').onsubmit = async e => {
      ...
      if(!p || p.length < 8) return status('A senha precisa ter 8 caracteres ou mais.', true, '#fcu-pwd-status');
      status('Atualizando senha...', false, '#fcu-pwd-status');
      const res = await db.auth.updateUser({ password: p });

...
Este repositório preserva o Dashboard V2 e acrescenta um piloto de autenticação:

- uma célula de demonstração por navegador;
- login obrigatório a partir de outra célula;
- cadastro com nome, e-mail, instituição e aceite versionado;
- confirmação de e-mail, senha, recuperação e encerramento de sessão via Supabase Auth;
- eventos autenticados associados à conta somente depois do login;
- páginas públicas de termos e privacidade.

O frontend usa apenas a chave publicável do projeto `PreditorFCUMaster`. Credenciais SMTP, chave secreta do Supabase e chave do Resend não pertencem ao repositório.

Antes de publicar um novo endereço, configure-o em **Supabase → Authentication → URL Configuration** como Site URL e Redirect URL permitida.
